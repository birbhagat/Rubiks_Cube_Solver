...
#ifndef RUBIKSCUBESOLVERV1_0_0_CUBEBFS_H
#define RUBIKSCUBESOLVERV1_0_0_CUBEBFS_H
...
private:
    ...
    unordered_map<T, GenericCube::MOVE, H> move_done;
...
//    move_done[] -> Back-Pointer map as to how we reached that state
    T bfs() {
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < 18; i++) {
                auto curr_move = GenericCube::MOVE(i);
                node.move(curr_move);
                if (!visited[node]) {
                    visited[node] = true;
...
//    Performs BFS and returns the vector of moves done to solve the cube
    vector<GenericCube::MOVE> solve() {
        ...
        while (!(curr_cube == rubiksCube)) {
            GenericCube::MOVE curr_move = move_done[curr_cube];
...
#endif //RUBIKSCUBESOLVERV1_0_0_CUBEBFS_H
