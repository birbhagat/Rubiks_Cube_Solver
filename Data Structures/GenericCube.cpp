...
char GenericCube::getColorLetter(COLOR color) {
    switch (color) {
        case COLOR::BLUE:
            return 'B';
        case COLOR::GREEN:
 ...
 */
string GenericCube::getMove(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return "L";
        case MOVE::LPRIME:
 ...
 */
GenericCube &GenericCube::move(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->l();
        case MOVE::LPRIME:
 ...
 * Invert a move.
 */
GenericCube &GenericCube::invert(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->lPrime();
        case MOVE::LPRIME:
...
vector<GenericCube::MOVE> GenericCube::randomShuffleCube(unsigned int times) {
...
string GenericCube::getCornerColorString(uint8_t ind) const {
    string str = "";
...
uint8_t GenericCube::getCornerIndex(uint8_t ind) const {
...
uint8_t GenericCube::getCornerOrientation(uint8_t ind) const {
    string corner = getCornerColorString(ind);
